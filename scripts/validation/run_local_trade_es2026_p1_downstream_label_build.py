#!/usr/bin/env python3
"""Run the approved ES 2026 downstream Phase 3 label build."""
...
from scripts.validation import build_local_trade_es2026_p1_downstream_label_gate as gate  # noqa: E402


STAGE = "local_trade_es2026_p1_downstream_label_build_execution"
STATUS_DRY_RUN_READY = "DRY_RUN_READY_ES2026_P1_DOWNSTREAM_LABEL_BUILD"
STATUS_EXECUTED = "EXECUTED_ES2026_P1_DOWNSTREAM_LABEL_BUILD"
STATUS_NO_GO = "NO_GO_ES2026_P1_DOWNSTREAM_LABEL_BUILD_EXECUTION"
DECISION_DRY_RUN = "es2026_p1_downstream_label_build_execution_dry_run_only"
DECISION_APPROVED = "human_approved_es2026_p1_downstream_label_build_execution"
DECISION_BLOCKED = "es2026_p1_downstream_label_build_execution_blocked"
...
def _approval_flags(*, executed: bool) -> dict[str, bool]:
    ...
    if executed:
        flags["label_build_approved"] = True
    return flags
...
def _command_bound_failures(
    ...
    *,
    ...
    if executable_tail != ["-m", "scripts.phase3_labels.build_labels"]:
        failures.append("command does not invoke scripts.phase3_labels.build_labels")
    for flag in gate.FORBIDDEN_LABEL_FLAGS:
        if flag in argv_set:
            failures.append(f"forbidden label argument present: {flag}")
    expected_values = {
        "--profile": TARGET_PROFILE,
        "--input-root": rel(candidate_causal_root, repo_root),
        "--output-root": rel(label_output_root, repo_root),
        "--reports-root": rel(label_reports_root, repo_root),
        "--costs-config": rel(costs_config, repo_root),
        "--profile-config": rel(profile_config, repo_root),
        "--causal-base-manifest": rel(build_reports_root / "causal_base_manifest.json", repo_root),
        "--accepted-readiness-exceptions": rel(profile_config, repo_root),
...
def _validate_label_outputs(
    *,
    ...
    manifest_path = label_reports_root / "label_manifest.json"
    report_path = label_reports_root / "label_report.json"
    ...
    expected_pairs = [{"market": TARGET_MARKET, "year": TARGET_YEAR}]
    if manifest.get("stage") != "labels":
        failures.append(f"label manifest stage is not labels: {manifest.get('stage')!r}")
    if manifest.get("status") != "PASS":
        failures.append(f"label manifest status is not PASS: {manifest.get('status')!r}")
    if manifest.get("profile") != TARGET_PROFILE:
        failures.append(f"label manifest profile mismatch: {manifest.get('profile')!r}")
    if manifest.get("resolved_profile") != TARGET_PROFILE:
        failures.append(
            f"label manifest resolved_profile mismatch: {manifest.get('resolved_profile')!r}"
        )
    if manifest.get("markets") != [TARGET_MARKET]:
        failures.append(f"label manifest markets mismatch: {manifest.get('markets')!r}")
    if manifest.get("years") != [TARGET_YEAR]:
        failures.append(f"label manifest years mismatch: {manifest.get('years')!r}")
    if not _path_matches(manifest.get("input_root"), candidate_causal_root, repo_root):
        failures.append("label manifest input_root mismatch")
    if not _path_matches(manifest.get("output_root"), label_output_root, repo_root):
        failures.append("label manifest output_root mismatch")
    if not _path_matches(manifest.get("reports_root"), label_reports_root, repo_root):
        failures.append("label manifest reports_root mismatch")
    if manifest.get("failure_count") not in (0, None):
        failures.append(f"label manifest failure_count is not 0: {manifest.get('failure_count')!r}")
    if manifest.get("failures") not in ([], None):
        failures.append("label manifest failures are non-empty")

    selection = manifest.get("input_selection")
    if not isinstance(selection, Mapping):
        failures.append("label manifest input_selection missing")
    else:
        expected_selection = {
        ...
        for key, expected in expected_selection.items():
            if selection.get(key) != expected:
                failures.append(f"label manifest input_selection.{key} mismatch")

    causal_gate = manifest.get("causal_base_manifest_gate")
    if not isinstance(causal_gate, Mapping):
        failures.append("label manifest causal_base_manifest_gate missing")
    else:
        if causal_gate.get("status") != "PASS":
            failures.append(
                "label manifest causal_base_manifest_gate.status is not PASS: "
                f"{causal_gate.get('status')!r}"
            )
        if not _path_matches(
            causal_gate.get("manifest_path"),
            build_reports_root / "causal_base_manifest.json",
            repo_root,
        ):
            failures.append("label manifest causal_base_manifest_gate.manifest_path mismatch")
        if causal_gate.get("expected_market_year_count") != 1:
            failures.append("label manifest causal_base_manifest_gate expected count mismatch")
        if causal_gate.get("accepted_readiness_exception_count") != 1:
            failures.append(
                "label manifest causal_base_manifest_gate accepted_readiness_exception_count mismatch"
            )
        if causal_gate.get("accepted_readiness_exceptions_path") != rel(
            accepted_readiness_exceptions,
            repo_root,
        ):
            failures.append(
                "label manifest causal_base_manifest_gate accepted_readiness_exceptions_path mismatch"
            )

    outputs = manifest.get("outputs")
    if not isinstance(outputs, list) or len(outputs) != 1:
        failures.append("label manifest outputs must contain exactly one ES 2026 row")
    else:
        output = outputs[0]
        if not isinstance(output, Mapping):
            failures.append("label manifest output row is not an object")
        else:
            if {"market": output.get("market"), "year": output.get("year")} not in expected_pairs:
                failures.append("label manifest output row market/year mismatch")
            if output.get("status") != "PASS":
                failures.append(f"label manifest output status is not PASS: {output.get('status')!r}")
            if output.get("failure_count") not in (0, None):
                failures.append("label manifest output failure_count is not 0")
            if output.get("failures") not in ([], None):
                failures.append("label manifest output failures are non-empty")
            if not _path_matches(output.get("output_path"), output_path, repo_root):
                failures.append("label manifest output path mismatch")

    expected_hash = _matching_output_hash(manifest, output_path, repo_root)
    if expected_hash is None:
        failures.append("label manifest output hash missing")
    elif output_path.exists() and file_sha256(output_path) != expected_hash:
        failures.append("label manifest output hash stale")

    if report_error is None:
        if report.get("stage") != "labels":
            failures.append(f"label report stage is not labels: {report.get('stage')!r}")
        if report.get("status") != "PASS":
            failures.append(f"label report status is not PASS: {report.get('status')!r}")
        if report.get("markets") != [TARGET_MARKET] or report.get("years") != [TARGET_YEAR]:
            failures.append("label report market/year scope mismatch")
    return failures


def _recommended_next(status: str, *, execute: bool) -> str:
    if status == STATUS_NO_GO:
        return "Resolve failed ES 2026 downstream label build wrapper checks, then rerun this guarded wrapper."
    if execute:
        return (
            "Review generated ES 2026 label evidence, then prepare a separate bounded feature-matrix "
            "approval gate; do not run features or modeling without separate approval."
        )
    return (
        "Approve or leave pending this guarded ES 2026 Phase 3 label wrapper; do not run labels "
        "or features without explicit approval."
    )
    ...
    *,
    ...
    report = dict(
        ...
        if gate_report is not None
            ...
            repo_root=repo_root,
            candidate_causal_root=candidate_causal_root,
    ...
    if not exact_command:
        exact_command = gate._label_command(
            input_root=Path(rel(candidate_causal_root, repo_root)),
            output_root=Path(rel(label_output_root, repo_root)),
            reports_root=Path(rel(label_reports_root, repo_root)),
            causal_base_manifest=Path(rel(build_reports_root / "causal_base_manifest.json", repo_root)),
            profile_config=Path(rel(profile_config, repo_root)),
            costs_config=Path(rel(costs_config, repo_root)),
            accepted_readiness_exceptions=Path(rel(profile_config, repo_root)),
        )
    argv = _command_to_argv(exact_command)
    ...
    stale_root_files = sorted(
        {
            *_existing_tree_files(repo_root, label_output_root, "data"),
            *_existing_tree_files(repo_root, label_reports_root, "reports"),
        }
    )
    expected_path_set = {rel(path, repo_root) for path in expected_paths}
    completed_outputs_present = (
        bool(expected_path_set)
        and set(existing_expected_paths) == expected_path_set
        and set(stale_root_files) <= expected_path_set
    )
    completed_output_failures = (
        _validate_label_outputs(
            repo_root=repo_root,
            ...
            label_reports_root=label_reports_root,
            build_reports_root=build_reports_root,
            accepted_readiness_exceptions=profile_config,
        )
        if completed_outputs_present
        else []
    )
    completed_outputs_valid = completed_outputs_present and not completed_output_failures
    command_failures = (
        ...
        if argv
        else ["missing exact label command"]
    )

    checks: list[dict[str, Any]] = []
    _check(
        checks,
        name="downstream_label_gate_ready",
        passed=gate_summary.get("status") == gate.STATUS_READY or completed_outputs_valid,
        observed=gate_summary.get("status"),
        expected=f"{gate.STATUS_READY}, or exact completed label outputs already validated",
        detail="Execution must start from the console-only downstream label approval gate unless the approved output set already exists and validates.",
    )
    _check(
        checks,
        ...
        detail="Generated label artifacts require explicit approval.",
    )
    _check(
        checks,
        name="bounded_phase3_label_command_only",
        passed=not command_failures,
        observed=command_failures,
        expected="exact ES 2026 Phase 3 label command from the approval gate",
        detail="Execution must remain bounded to ES 2026 candidate labels.",
    )
    _check(
        checks,
        ...
        detail="Label artifacts must stay in generated artifact roots.",
    )
    _check(
        checks,
        ...
        expected=[],
        detail="Label outputs must be ignored generated artifacts before execution.",
    )
    _check(
        checks,
        name="planned_outputs_absent_before_execution",
        passed=not existing_expected_paths or completed_outputs_valid,
        observed=existing_expected_paths,
        expected=[],
        detail="Before execution, planned label outputs must be absent; after approved execution, the exact planned outputs may exist and validate.",
    )
    _check(
        checks,
        name="candidate_label_roots_empty_before_execution",
        passed=not stale_root_files or completed_outputs_valid,
        observed=stale_root_files,
        expected=[],
        detail="Candidate label roots should be empty before execution or contain only validated planned outputs after execution.",
    )
    _check(
        checks,
        ...
        expected=[],
        detail="Generated data/** and reports/** artifacts must not be staged before label execution.",
    )

    command_results: list[dict[str, Any]] = []
    output_failures: list[str] = list(completed_output_failures)
    unexpected_outputs: list[str] = (
        sorted(path for path in stale_root_files if path not in expected_path_set)
        if completed_outputs_present
        else []
    )
    ...
    if execute and not failures:
        ...
        try:
            ...
            command_results.append(
                {
                    "name": "phase3_label_build_exact_es2026_candidate",
                    "returncode": None,
                    "timed_out": True,
                    "stdout": exc.stdout,
                    "stderr": exc.stderr,
        ...
        if command_results and command_results[-1].get("returncode") == 0:
            output_failures = _validate_label_outputs(
                repo_root=repo_root,
                expected_paths=expected_paths,
                candidate_causal_root=candidate_causal_root,
                label_output_root=label_output_root,
        ...
        _check(
            checks,
            name="phase3_label_command_completed",
            ...
            expected="Phase 3 label command returns 0 without timeout",
            detail="Stop on timeout, traceback, nonzero exit, manifest-gate failure, or cost/config failure.",
        )
        _check(
            checks,
            name="phase3_label_outputs_valid",
            passed=not output_failures,
            observed=output_failures,
            expected="PASS label manifest/report for exactly ES 2026",
            detail="Generated outputs must prove exact-scope ES 2026 label construction.",
        )
        _check(
            checks,
            name="unexpected_label_outputs_absent",
            passed=not unexpected_outputs,
            observed=unexpected_outputs,
            expected=[],
            detail="Execution may only create the planned ES 2026 label artifacts.",
        )
        _check(
            checks,
            ...
            expected=[],
            detail="Generated data/** and reports/** artifacts must remain unstaged after label execution.",
        )
    elif completed_outputs_present:
        _check(
            checks,
            name="phase3_label_outputs_valid",
            passed=not output_failures,
            observed=output_failures,
            expected="PASS label manifest/report for exactly ES 2026",
            detail="Existing completed outputs must prove exact-scope ES 2026 label construction.",
        )
        _check(
            checks,
            name="unexpected_label_outputs_absent",
            ...
            observed=unexpected_outputs,
            expected=[],
            detail="Completed label roots may contain only the planned ES 2026 label artifacts.",
        )
    ...
    elif execute or completed_outputs_valid:
    ...
    generated_outputs = [rel(path, repo_root) for path in expected_paths if executed and path.exists()]
    return {
        "summary": {
            "stage": STAGE,
        ...
        "non_approval": {
            "scope": "ES 2026 downstream Phase 3 label build wrapper",
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--candidate-causal-root", default=str(gate.DEFAULT_CANDIDATE_CAUSAL_ROOT))
    ...
    parser.add_argument("--build-reports-root", default=str(gate.DEFAULT_BUILD_REPORTS_ROOT))
    ...
    parser.add_argument("--label-script", default=str(gate.DEFAULT_LABEL_SCRIPT))
