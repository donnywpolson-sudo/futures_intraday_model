...
from scripts.validation import build_master_audit_phase10_reconciliation as phase10
...
def _phase8_decision_payload() -> dict[str, object]:
    return {
        ...
        "statistical_validity_gate": {"status": "FAIL"},
...
def _base_repo(
    ...
    *,
    ...
    _write_text(
        ...
        "Phase 10\nArtifact freeze\nblocked unless explicit freeze approval\n",
    )
    _write_text(
    ...
    _write_text(tmp_path / "CODEX_HANDOFF.md", "Phase 10 report-only Master Audit reconciliation\n")
    payloads = _payloads()
    if payload_overrides:
        payloads.update(payload_overrides)
    _write_json(tmp_path / phase10.DEFAULT_RUN_STATUS, payloads.get("run_status", _run_status_payload()))
    _write_json(tmp_path / phase10.DEFAULT_OVERVIEW, payloads.get("overview", _overview_payload()))
    for key, path in phase10.REQUIRED_INPUTS.items():
        if key in {"master_audit", "project_outline", "codex_handoff", "run_status", "overview"}:
            continue
        _write_json(tmp_path / path, payloads[key])
    return tmp_path / phase10.DEFAULT_REPORTS_ROOT
...
def test_phase10_reconciliation_passes_as_na_not_approved_freeze(tmp_path: Path) -> None:
    report = _build(tmp_path)

    assert report["status"] == phase10.PASS_STATUS
    assert report["summary"]["phase10_master_audit_status"] == "N/A_NOT_APPROVED_ARTIFACT_FREEZE"
    assert report["summary"]["phase10_full_master_audit_accepted"] is False
    assert report["summary"]["artifact_freeze_ready"] is False
    assert report["summary"]["artifact_freeze_allowed"] is False
    assert report["summary"]["freeze_executed"] is False
...
def test_phase10_ledger_upgrade_fails_closed(tmp_path: Path) -> None:
    bad_run_status = _run_status_payload()
    bad_run_status["run_status_table"][0]["run_status"] = "RUN"
    bad_run_status["run_status_table"][0]["detail_status"] = "RUN_FREEZE_APPROVED"

    report = _build(tmp_path, payload_overrides={"run_status": bad_run_status})

    assert report["status"] == phase10.FAIL_STATUS
    assert any("Phase 10 row" in failure for failure in report["failures"])
...
def test_upstream_phase9_upgrade_fails_closed(tmp_path: Path) -> None:
    payloads = _previous_phase_payloads()
    bad_phase9 = copy.deepcopy(payloads["phase9_reconciliation"])
    bad_phase9["summary"]["promotion_allowed"] = True

    report = _build(tmp_path, payload_overrides={"phase9_reconciliation": bad_phase9})

    assert report["status"] == phase10.FAIL_STATUS
    assert any("Phase 1B-9" in failure for failure in report["failures"])


def test_upstream_freeze_flag_true_fails_closed(tmp_path: Path) -> None:
    payloads = _previous_phase_payloads()
    bad_phase8 = copy.deepcopy(payloads["phase8_reconciliation"])
    bad_phase8["non_approval"]["freeze_executed"] = True
...
def test_main_writes_only_json_and_markdown_reports(tmp_path: Path) -> None:
    ...
    payload = json.loads((reports_root / phase10.REPORT_JSON).read_text(encoding="utf-8"))
    assert payload["summary"]["artifact_freeze_ready"] is False
    assert payload["non_approval"]["artifact_freeze_command_executed"] is False


def test_invalid_output_root_fails_closed(tmp_path: Path) -> None:
    _base_repo(tmp_path)

    report = phase10.build_report(
        repo_root=tmp_path,
