...
def test_classify_session_normalized_candidate(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    _write(tmp_path / "src/ES/2025.parquet", _ohlcv().with_columns(pl.lit("s").alias("session_id"), pl.lit("2025-01-01").alias("session_date")))
    report = classify_checkpoint(tmp_path / "src")
    assert report["inferred_stage"] == "session_normalized_candidate"
    assert "causal timing columns missing" in report["reason"]
