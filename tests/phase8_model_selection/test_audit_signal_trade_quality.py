...
def test_signal_trade_quality_cli_predictions_has_no_implicit_default() -> None:
    args = build_arg_parser().parse_args([])

    assert args.predictions is None


def test_signal_trade_quality_cli_missing_predictions_fails_clearly(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(sys, "argv", ["audit_signal_trade_quality"])
...
def test_signal_trade_quality_cli_accepts_explicit_report_scoped_predictions(tmp_path: Path) -> None:
